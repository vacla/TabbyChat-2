...
public class GuiSettingsServer extends SettingPanel<ServerSettings> {
    ...
    GuiSettingsServer() {
        ...
        this.setDisplayString(I18n.translate(SETTINGS_SERVER));
    ...
    public void initGUI() {
        ...
        int pos = 1;
        this.add(new GuiLabel(new TranslatableText(CHANNELS_ENABLED)), new int[]{2, pos});
        GuiSettingBoolean chkChannels = new GuiSettingBoolean(sett.channelsEnabled);
        chkChannels.setCaption(new TranslatableText(CHANNELS_ENABLED_DESC));
        this.add(chkChannels, new int[]{1, pos});

        pos += 1;
        this.add(new GuiLabel(new TranslatableText(PM_ENABLED)), new int[]{2, pos});
        GuiSettingBoolean chkPM = new GuiSettingBoolean(sett.pmEnabled);
        chkPM.setCaption(new TranslatableText(PM_ENABLED_DESC));
        this.add(chkPM, new int[]{1, pos});

        pos += 1;
        add(new GuiLabel(new TranslatableText(USE_DEFAULT)), new int[]{2, pos});
        add(new GuiSettingBoolean(sett.useDefaultTab), new int[]{1, pos});

        pos += 2;
        this.add(new GuiLabel(new TranslatableText(CHANNEL_PATTERN)), new int[]{1, pos});
        GuiSettingEnum<ChannelPatterns> enmChanPat = new GuiSettingEnum<>(sett.channelPattern,
                ChannelPatterns.values());
        enmChanPat.setCaption(new TranslatableText(CHANNEL_PATTERN_DESC));
        this.add(enmChanPat, new int[]{5, pos, 4, 1});

        pos += 2;
        this.add(new GuiLabel(new TranslatableText(MESSAGE_PATTERN)), new int[]{1, pos});
        ...
        enmMsg.setCaption(new TranslatableText(MESSAGE_PATTERN_DESC));
        this.add(enmMsg, new int[]{5, pos, 4, 1});

        pos += 2;
        this.add(new GuiLabel(new TranslatableText(IGNORED_CHANNELS)), new int[]{0, pos});
        GuiSettingStringList strIgnored = new GuiSettingStringList(sett.ignoredChannels);
        strIgnored.setCaption(new TranslatableText(IGNORED_CHANNELS_DESC));
        this.add(strIgnored, new int[]{5, pos, 5, 1});

        pos += 2;
        this.add(new GuiLabel(new TranslatableText(DEFAULT_CHANNEL_COMMAND)), new int[]{0, pos});
        GuiSettingString strChannels = new GuiSettingString(sett.channelCommand);
        strChannels.setCaption(new TranslatableText(DEFAULT_CHANNEL_COMMAND_DESC));
        this.add(strChannels, new int[]{5, pos, 5, 1});

        pos += 2;
        this.add(new GuiLabel(new TranslatableText(DEFAULT_CHANNEL)), new int[]{0, pos});
        GuiSettingString strMessages = new GuiSettingString(sett.defaultChannel);
        strMessages.setCaption(new TranslatableText(DEFAULT_CHANNEL_DESC));
